...
class RoundState:
    def __init__(self) -> None:
        self.__xiangting_calculator = Shanten()
    ...
    def on_zimo(
            ...
            for i in range(len(self.__my_hand)):
                ...
                if len(self.__my_fulu_list) == 0 and my_score >= 1000:
                    ...
                    for i in range(34):
                        ...
                        array34[i] += 1
                        if self.__xiangting_calculator.calculate_shanten(array34) == -1:
        ...
        if self.__xiangting_calculator.calculate_shanten(self.hand_to_34_array(combined_hand)) == -1:
    ...
    def on_dapai(
        ...
        xiangting_number = self.__xiangting_calculator.calculate_shanten(self.hand_to_34_array(combined_hand))
        if xiangting_number == -1 and (tile not in self.__get_my_zhenting_tiles(seat)) and self.__my_zhenting == 0:
            # ロンが出来るかどうかチェックする．
            player_wind = (seat + 4 - self.__index) % 4
            has_yihan = self.__hand_calculator.has_yihan(
                self.__chang, player_wind, self.__my_hand, self.__my_fulu_list,
...
class Kanachan:
    def __init__(self, model_path="./model/model.kanachan") -> None:
